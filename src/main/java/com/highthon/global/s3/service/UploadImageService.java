package com.highthon.global.s3.service;

import com.highthon.global.error.GlobalException;
import com.highthon.global.s3.properties.S3Environment;
...
public class UploadImageService {
    ...
    public String execute(MultipartFile multipartFile) {
        if (multipartFile.isEmpty()) {
            throw new GlobalException("파일이 존재하지 않습니다.", HttpStatus.BAD_REQUEST);
        }
    ...
    private void validateFileContentType(String fileExtension) {
        ...
        if (!allowExtension.contains(fileExtension.toLowerCase())) {
            throw new GlobalException("지원하지 않는 파일 확장자 입니다.", HttpStatus.BAD_REQUEST);
        }
    }
}
